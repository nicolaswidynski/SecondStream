...
private extension WebViewController {
	...
	func loadWebView(replaceExistingWebView: Bool = false) {
		...
		coordinator.webViewProvider.dequeueWebView { webView in
			...
			webView.ready {
				...
				// isOpaque, backgroundColor and scrollView.backgroundColor are set in PreloadedWebView.init
	...
	func renderPage(_ webView: PreloadedWebView?) {
		...
		let substitutions = [
			...
			"collapsibleSectionsEnabled": AppDefaults.shared.collapsibleArticleSectionsEnabled ? "true" : "false"
		]
...
//		print("article.html written to \(fileURL.path)")
	...
	func toggleStarredAction() -> UIAction {
		...
		let title = starred ? NSLocalizedString("Remove Bookmark", comment: "Remove Bookmark") : NSLocalizedString("Bookmark", comment: "Bookmark")
