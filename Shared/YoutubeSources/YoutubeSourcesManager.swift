...
@MainActor final class YoutubeSourcesManager {
	...
	private static let topFileName = "yt_top.txt"
	private static let libraryFileName = "yt.txt"
	...
	private func fetchYoutubeSources() async {
		...
		if let entries = await topEntries {
			let sources = entries.map { YoutubeSource(name: $0.name, author: $0.author, url: $0.rssURL, myFeedURL: $0.myFeedURL, imageURL: $0.imageURL) }
