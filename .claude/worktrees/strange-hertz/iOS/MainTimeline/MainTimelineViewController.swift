...
@MainActor enum FeedNavigationChrome {
	...
	static func makeTopBarFeedIcon(from iconImage: IconImage, isPseudoFeedIcon: Bool, cacheKey: String? = nil) -> UIImage {
		...
		func renderCircle(from image: UIImage, trait: UITraitCollection) -> UIImage {
			UIGraphicsImageRenderer(size: canvasSize, format: .init(for: trait)).image { _ in
		...
		// When a light variant is available, pick the correct source image for the
		// current trait and skip the cache — the caller clears lastNavigationIconKey
		// on trait change so this re-renders automatically.
		if let lightSourceImage = iconImage.lightImage {
			let isLight = UITraitCollection.current.userInterfaceStyle == .light
			let sourceImage = isLight ? lightSourceImage : iconImage.image
			return renderCircle(from: sourceImage, trait: UITraitCollection.current)
		}

		let result = renderCircle(from: iconImage.image, trait: .current)
		if let namespacedCacheKey {
